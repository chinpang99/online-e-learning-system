      ...
      <div class="container">
        <div class="row title-bar">
          ...
          <?php
            ...
            <h1 class="wow fadeInUp">Chapter <?php echo $rows['chapter'] ?> :  <?php echo $rows['chap_name'] ?></h1>
        ...
        <?php
        	...
        	<?php
        		...
        		if($rows = mysqli_fetch_array($query))
        		{
        			...
        			$sql_questions = "SELECT *, @n := @n +1 n FROM quiz_question INNER JOIN chapter ON quiz_question.chap_id = chapter.chap_id, (SELECT @n := 0) m WHERE chapter.chap_id = '$chap_id' ORDER BY quiz_question.question_id";
        			
        			$query_questions = mysqli_query($conn, $sql_questions);
        			
        			global $i;
        			
        			while($rows_questions = mysqli_fetch_array($query_questions))
        			{        				
        			...
        			echo "<a href='tutorial_questions.php?id=$c' class='btn btn-green' title='$b' role='button'>";
        			?>
            			<i class="fa"><?php echo $b ?></i>
        			</a>
