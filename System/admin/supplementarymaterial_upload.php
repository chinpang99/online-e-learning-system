<?php
	...
	$filename = basename($_FILES['material_file']['name']); //result = abc.jpg
	
	//step 2: get the file size
	$checkfilesize = $_FILES['material_file']['size'];
	...
	$destination = "material/".$filename; //follow the previous item name
	
	//extra: if you want to change the item name:
	//e.g. $destination = "profit/".ic.".".$getFileType;
	
	//check for filesize whether more than 10MB
	...
	if($getFileType != "pdf" && $getFileType != "docx" )
	{
		echo"<script>alert('Sorry file is not correct. Please choose others file format!');</script>";
		die("<script>window.history.go(-1);</script>");
	}
